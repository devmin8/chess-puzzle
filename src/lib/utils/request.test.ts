...
describe('request', () => {
	test('returns the parsed body when the response is ok', async () => {
		...
		await expect(request('/api/example', { method: 'POST' })).resolves.toEqual({
			ok: true,
			result: { id: 7 }
		});
	});
	...
	test('returns a network error when fetch rejects', async () => {
		...
		const outcome = await request('/api/example');
	...
	test('returns the server message when the response is not ok', async () => {
		vi.stubGlobal(
			'fetch',
			vi.fn().mockResolvedValue(jsonResponse({ message: 'Already exists' }, 400))
		);
		...
		expect(outcome).toEqual({
			ok: false,
			result: undefined,
			error: { kind: 'http', message: 'Already exists', status: 400 }
		});
	});
